...
import com.alpha.finology.error.ErrorMessageConstant;
...
import com.alpha.finology.validation.annotation.SortProperty;
...
public class SpecificationHelper {
  ...
  public static <T> Specification<T> inJobPositionChildrenByJobPositionId(String column,
    ...
    if (jobPositionParentId == null || jobPositionParentId.isBlank()) {
      ...
      return ((root, query, cb) -> {
        Path<T> path = getRootPath(column, root);
        return path.in(cb.function("get_job_position_children_direct_indirect", String.class,
            cb.<String>literal(jobPositionParentId)));
      });
    }
  }

  public static <T> Specification<T> inJobPositionDirectChildrenByJobPositionId(String column,
      String jobPositionParentId) {
    if (jobPositionParentId == null || jobPositionParentId.isBlank()) {
      return ((root, query, cb) -> cb.disjunction());
    } else {
      return ((root, query, cb) -> {
        Path<T> path = getRootPath(column, root);
        return path.in(cb.function("get_job_position_children_direct", String.class,
            cb.<String>literal(jobPositionParentId)));
      });
    }
  }

  public static <T> Specification<T> inJobPositionIndirectChildrenByJobPositionId(String column,
      String jobPositionParentId) {
    if (jobPositionParentId == null || jobPositionParentId.isBlank()) {
      return ((root, query, cb) -> cb.disjunction());
    } else {
      return ((root, query, cb) -> {
        Path<T> path = getRootPath(column, root);
        return path.in(cb.function("get_job_position_children_indirect", String.class,
            cb.<String>literal(jobPositionParentId)));
      });
    }
  }
  ...
  public static void addDefaultSort(PagingRequest pagingRequest, String propertyName) {
    if (pagingRequest.getSortBy() == null || pagingRequest.getSortBy().isEmpty()) {
      pagingRequest.setSortBy(Arrays.asList(new SortBy(propertyName, SortByDirection.ASC)));
    }
  }
}